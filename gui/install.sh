...
echo -e "   ${WHITE}↓${RESET} Téléchargement du GIF animé..."
download "$GIF_URL" "$INSTALL_DIR/icons/xxmr.gif" 2>/dev/null && \
    echo -e "   ${GREEN}✓${RESET} xxmr.gif" || \
    echo -e "   ${ORANGE}~${RESET} GIF non téléchargé (optionnel)"

# Download QR code
echo -e "   ${WHITE}↓${RESET} Téléchargement du QR code..."
download "$QRCODE_URL" "$INSTALL_DIR/icons/qrcode1.png" 2>/dev/null && \
    echo -e "   ${GREEN}✓${RESET} qrcode1.png" || \
    echo -e "   ${ORANGE}~${RESET} QR code non téléchargé (optionnel)"
