...
TRANSLATIONS = {
    "FR": {
        ...
        "current_datadir": "Emplacement actuel :",
...
🔗 Code source: <a href="https://github.com/kerlannXmr/EasyMonerod/blob/main/gui/README.md">https://github.com/kerlannXmr/easymonerod/blob/main/gui/README.md</a>""",
    ...
    "EN": {
        ...
        "current_datadir": "Current location:",
...
class EasyMonerodApp(Adw.Application):
    ...
    def _build_welcome_page(self):
        ...
        # Vexillum ASCII centratumkerlannxmr
    ...
    def _build_app_ui(self):
        ...
        # Bouton lien vers Xmr.Ecosystem
    ...
    def _build_external_disk_page(self):
        """Gradus 6: Selectio directorii pro blockchain — internus aut explorator"""
        page = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=16)
        page.set_margin_start(24)
        page.set_margin_end(24)
        page.set_margin_top(20)
        ...
        info_lbl = Gtk.Label()
        info_lbl.set_markup(
            "<b>Cette étape permet de choisir où stocker la blockchain :</b>\n"
            "  • <b>Disque interne</b> : dossier par défaut ~/.bitmonero\n"
            "  • <b>Autre dossier</b> : disque externe, SSD, NAS, etc.\n"
            "  • Configure <b>data-dir</b> dans bitmonero.conf\n"
            "  • ⚠️ Le dossier choisi doit être accessible en écriture"
            if self.lang == "FR" else
            "<b>This step lets you choose where to store the blockchain:</b>\n"
            "  • <b>Internal disk</b>: default folder ~/.bitmonero\n"
            "  • <b>Custom folder</b>: external disk, SSD, NAS, etc.\n"
            "  • Sets <b>data-dir</b> in bitmonero.conf\n"
            "  • ⚠️ The chosen folder must be writable"
        )
        info_lbl.set_xalign(0)
        info_lbl.set_wrap(True)
        info.append(info_lbl)
        ...
        current_card = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6)
        current_card.add_css_class("option-card")

        current_title = Gtk.Label(label=self.t("current_datadir"))
        current_title.set_xalign(0)
        current_title.add_css_class("page-subtitle")
        current_card.append(current_title)

        self._current_datadir_label = Gtk.Label(label="...")
        self._current_datadir_label.set_xalign(0)
        self._current_datadir_label.set_selectable(True)
        self._current_datadir_label.add_css_class("status-running")
        current_card.append(self._current_datadir_label)

        page.append(current_card)

        # Lectio data-dir actualis
        GLib.idle_add(self._load_current_datadir)

        # ═══ 2) Choix disque INTERNE (chemin par défaut) ═══
        internal_card = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8)
        internal_card.add_css_class("option-card")

        self._folder_radio_internal = Gtk.CheckButton()
        internal_label_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)

        internal_title = Gtk.Label()
        internal_title.set_markup(
            "<b>🖥️ " + ("Disque interne (par défaut)" if self.lang == "FR"
                          else "Internal disk (default)") + "</b>")
        internal_title.set_xalign(0)
        internal_label_box.append(internal_title)

        self._internal_path_label = Gtk.Label()
        default_path = f"/home/{self.username}/.bitmonero"
        self._internal_path_label.set_text(f"📂 {default_path}")
        self._internal_path_label.set_xalign(0)
        self._internal_path_label.set_opacity(0.7)
        internal_label_box.append(self._internal_path_label)

        self._folder_radio_internal.set_child(internal_label_box)
        self._folder_radio_internal.set_active(True)
        internal_card.append(self._folder_radio_internal)

        page.append(internal_card)

        # ═══ 3) Choix AUTRE DOSSIER (explorateur) ═══
        custom_card = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8)
        custom_card.add_css_class("option-card")

        self._folder_radio_custom = Gtk.CheckButton()
        self._folder_radio_custom.set_group(self._folder_radio_internal)

        custom_title = Gtk.Label()
        custom_title.set_markup(
            "<b>📂 " + ("Autre dossier (disque externe, SSD, NAS...)" if self.lang == "FR"
                         else "Custom folder (external disk, SSD, NAS...)") + "</b>")
        custom_title.set_xalign(0)
        self._folder_radio_custom.set_child(custom_title)
        custom_card.append(self._folder_radio_custom)

        # Sous-bloc parcourir — visible uniquement si "Autre dossier" sélectionné
        self._browse_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8)
        self._browse_box.set_margin_start(28)
        self._browse_box.set_margin_top(8)

        # Linea: [Entry chemin] [Bouton Parcourir]
        path_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)

        self.folder_entry = Gtk.Entry()
        self.folder_entry.set_placeholder_text(
            "/media/user/disque/MoneroBlockchain" if self.lang == "FR"
            else "/media/user/disk/MoneroBlockchain")
        self.folder_entry.set_hexpand(True)
        path_box.append(self.folder_entry)
        ...
        self._browse_box.append(path_box)

        # Informationes de directorio selecti (spatium, permissiones)
        self._folder_info_label = Gtk.Label(label="")
        self._folder_info_label.set_xalign(0)
        self._folder_info_label.set_wrap(True)
        self._folder_info_label.set_margin_top(4)
        self._browse_box.append(self._folder_info_label)

        self.folder_entry.connect("changed", lambda e: self._update_folder_info(e.get_text()))

        # Initio occultatum
        self._browse_box.set_visible(False)
        custom_card.append(self._browse_box)

        page.append(custom_card)

        # Commutatio visibilitatis browse_box
        self._folder_radio_custom.connect("toggled",
            lambda r: self._browse_box.set_visible(r.get_active()))
        self._folder_radio_internal.connect("toggled",
            lambda r: self._browse_box.set_visible(not r.get_active()))
        ...
        self._folder_status_label = Gtk.Label(label="")
        self._folder_status_label.set_hexpand(True)
        self._folder_status_label.set_xalign(0)
        btn_box.append(self._folder_status_label)

        run_btn = Gtk.Button(label="▶  " + self.t("confirm"))
        run_btn.add_css_class("action-button")
        run_btn.connect("clicked", lambda b: self._apply_folder_datadir(b))
        btn_box.append(run_btn)
    ...
    def _load_current_datadir(self):
        """Lectio data-dir actualis e bitmonero.conf"""
        config_path = f"/home/{self.username}/.bitmonero/bitmonero.conf"
        current = f"/home/{self.username}/.bitmonero"  # Valor defectus
        try:
        ...
        self._current_datadir_label.set_text(f"📂 {current}")
        return False

    def _on_browse_folder(self, button):
        """Aperire exploratorem archivorum GTK4 pro selectione directorii"""
        try:
            # GTK 4.10+ — Gtk.FileDialog (modus modernus)
            dialog = Gtk.FileDialog()
            dialog.set_title(self.t("select_folder_title"))

            # Positio initialis: /media (disci externi) aut domus usoris
            initial_path = f"/media/{self.username}"
            if not os.path.isdir(initial_path):
                initial_path = os.path.expanduser("~")
            initial_folder = Gio.File.new_for_path(initial_path)
            dialog.set_initial_folder(initial_folder)

            dialog.select_folder(self.win, None, self._on_folder_selected)

        except AttributeError:
            # GTK < 4.10 — substitutio cum Gtk.FileChooserNative
            self._open_folder_chooser_legacy()

    def _open_folder_chooser_legacy(self):
        """Substitutio pro GTK < 4.10 sine FileDialog"""
    ...
    def _on_folder_chooser_legacy_response(self, dialog, response):
        """Revocatio pro FileChooserNative (GTK < 4.10)"""
    ...
    def _on_folder_selected(self, dialog, result):
        """Revocatio cum directorium selectum est (GTK 4.10+ FileDialog)"""
        try:
            ...
            # Usor cancellavit — nihil facere
            pass

    def _update_folder_info(self, path):
        """Renovatio informationum de directorio selecti (spatium, permissiones)"""
        if not path or not os.path.isdir(path):
            self._folder_info_label.set_text("")
            return
        ...
        try:
            ...
            if self.lang == "FR":
                info_text = (f"💾 Espace libre : {free_gb:.1f} Go / {total_gb:.1f} Go")
                if writable:
                    info_text += "    ✅ Écriture OK"
                else:
                    info_text += "    ❌ Pas de permission d'écriture"
            else:
                info_text = (f"💾 Free space: {free_gb:.1f} GB / {total_gb:.1f} GB")
                if writable:
                    info_text += "    ✅ Writable"
                else:
                    info_text += "    ❌ No write permission"

            self._folder_info_label.set_text(info_text)
        except Exception:
            self._folder_info_label.set_text("")

    def _apply_folder_datadir(self, run_btn):
        """Applicatio directorii selecti ad bitmonero.conf (data-dir=)"""
        # Determinare viam secundum radio selectum
        if self._folder_radio_internal.get_active():
            # Modus internus: via defecta
            folder_path = f"/home/{self.username}/.bitmonero"
        else:
            # Modus personalizatus: via ex campo introitus
            folder_path = self.folder_entry.get_text().strip()
            if not folder_path:
                self._folder_status_label.set_text(self.t("folder_error_empty"))
                self._folder_status_label.add_css_class("status-warning")
                return

        run_btn.set_sensitive(False)
        self._folder_status_label.set_text("⏳ " + self.t("running"))
        self._folder_status_label.remove_css_class("status-warning")
        self._folder_status_label.add_css_class("status-running")
        ...
        cmd_str = (
            # Creare directorium destinationis si non existit
            f"mkdir -p '{escaped_folder}' && "
            f"chown -R {self.username}:{self.username} '{escaped_folder}' 2>/dev/null; "
            # Creare directorium .bitmonero si non existit
            f"mkdir -p \"$(dirname '{escaped_config}')\" && "
            f"chown -R {self.username}:{self.username} \"$(dirname '{escaped_config}')\" 2>/dev/null; "
            # Si bitmonero.conf existit, substituere data-dir; aliter creare
            f"if [ -f '{escaped_config}' ]; then "
            f"  sed -i '/^data-dir=/d' '{escaped_config}'; "
            f"  echo 'data-dir={escaped_folder}' >> '{escaped_config}'; "
            f"else "
            f"  echo 'data-dir={escaped_folder}' > '{escaped_config}'; "
            f"fi && "
            f"chown {self.username}:{self.username} '{escaped_config}' && "
            f"chmod 644 '{escaped_config}' && "
            f"echo '✅ data-dir={escaped_folder}' && "
            f"echo '✅ bitmonero.conf updated'"
        )
        ...
        def run_and_update():
            ...
            def update_ui():
                ...
                if success:
                    self._folder_status_label.set_text(self.t("folder_configured"))
                    self._folder_status_label.remove_css_class("status-warning")
                    self._folder_status_label.add_css_class("status-running")
                    # Renovare indicationem data-dir actualis
                    self._current_datadir_label.set_text(f"📂 {folder_path}")
                else:
                    err_msg = error if error else "Unknown error"
                    self._folder_status_label.set_text(f"⚠️ {err_msg}")
                    self._folder_status_label.remove_css_class("status-running")
                    self._folder_status_label.add_css_class("status-warning")
                return False
    ...
    def _update_sync_progress_bar(self, height, target, synced):
        """Renovatio barrae progressionis synchronizationis"""
    ...
    def _on_terminal_sync_output(self, terminal):
        ...
        def update_labels():
            ...
            # Renovatio barrae progressionis
    ...
    def _start_header_timers(self):
        ...
        # Nuntii: rotatio omni 10s, receptio RSS omni 30 min
        GLib.timeout_add_seconds(5,    self._rotate_news)   # incipit post primum receptum nuntiorum
    ...
    def _fetch_monero_news(self):
        """Receptio nuntiorum Monero per RSS: monero.forum + revuo-xmr.com"""
        import urllib.request
        import json
        ...
        def parse_rss_date(date_str):
            """Analysis datae RSS/Atom — retourne datetime aut None"""
            if not date_str:
                return None
        ...
        def fetch_revuo_news(source_icon="📰", max_items=25):
            """Lectio RSS Revuo → page HTML complète → extractio <h4> news"""
            items = []
            try:
                # 1) Lire le flux RSS pour trouver le lien du dernier numéro récent
                req = urllib.request.Request(
                ...
                for rss_item in root.findall(".//item"):
                    ...
                    if link_el is not None and link_el.text:
                        ...
                        break  # Premier numéro récent trouvé

                if not page_url:
                    return items

                # 2) Fetcher la page HTML complète du numéro
                ...
                # 3) Extraire les <h4> (chaque newsbyte = un titre de news)
