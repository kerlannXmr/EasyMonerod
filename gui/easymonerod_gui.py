...
Scriptum ab easynode_linux.sh v6 auctore kerlannXmr
...
/* Monero CLI up-to-date button */
.monero-uptodate-button {
    background: linear-gradient(135deg, #2a4a2a 0%, #1e3a1e 100%);
    color: #47a347;
    border: 1px solid #47a34760;
    border-radius: 8px;
    font-weight: 700;
}
.monero-uptodate-button:hover {
    background: linear-gradient(135deg, #325a32 0%, #2a4a2a 100%);
}
/* GUI update indicator (voyant) */
.gui-update-indicator {
    border-radius: 8px;
    padding: 6px 16px;
    margin: 4px 8px;
    font-size: 11px;
...
.gui-update-available {
    ...
    color: #ffffff;
    box-shadow: 0 0 12px rgba(255, 102, 0, 0.6), 0 0 4px rgba(255, 102, 0, 0.8);
    animation: gui-pulse 1.8s ease-in-out infinite;
}
@keyframes gui-pulse {
    0%, 100% { opacity: 1.0; box-shadow: 0 0 12px rgba(255, 102, 0, 0.6); }
    50%      { opacity: 0.75; box-shadow: 0 0 24px rgba(255, 102, 0, 0.9); }
}
.gui-update-ok {
    background-color: #47a34720;
    color: #47a347;
...
class EasyMonerodApp(Adw.Application):
    ...
    def _build_app_ui(self):
        ...
        # ── GUI update indicator (voyant) — thread launched later in _start_header_timers ──
        self.gui_update_indicator = Gtk.Label(
            label="⏳ " + ("Vérification GUI..." if self.lang == "FR" else "Checking GUI..."))
        self.gui_update_indicator.add_css_class("gui-update-indicator")
        self.gui_update_indicator.add_css_class("gui-update-checking")
        self.gui_update_indicator.set_xalign(0.5)
        sidebar.append(self.gui_update_indicator)
    ...
    def _start_header_timers(self):
        ...
        GLib.timeout_add_seconds(5, lambda: threading.Thread(target=self._check_gui_update, daemon=True).start() or False)
        return False
    ...
    def _show_up_to_date(self):
        ...
        self.update_btn.remove_css_class("secondary-button")
        self.update_btn.add_css_class("monero-uptodate-button")
        return False
    ...
    # ═══════════════════════════════════════════
    # GUI VERSION CHECK (voyant indicator)
    # ═══════════════════════════════════════════
    def _check_gui_update(self):
        """Background check: fetch remote GUI VERSION and update sidebar indicator."""
        remote_version = ""
        try:
            import urllib.request
            req = urllib.request.Request(
            ...
            try:
                ...
                pass
        GLib.idle_add(self._update_gui_indicator, remote_version)

    def _update_gui_indicator(self, remote_version):
        """Update the sidebar voyant based on remote version comparison."""
        # Remove all state classes
        for css_class in ["gui-update-checking", "gui-update-available",
                          "gui-update-ok", "gui-update-error"]:
            self.gui_update_indicator.remove_css_class(css_class)

        if not remote_version:
            self.gui_update_indicator.set_label(
                "⚠ GUI v" + APP_VERSION)
            self.gui_update_indicator.add_css_class("gui-update-error")
            self.gui_update_indicator.set_tooltip_text(
                "Impossible de vérifier la version distante"
                if self.lang == "FR" else
                "Unable to check remote version")
        elif remote_version != APP_VERSION:
            if self.lang == "FR":
                self.gui_update_indicator.set_label(
                    f"🔔Faire GUI Update → v{remote_version}")
            else:
                self.gui_update_indicator.set_label(
                    f"🔔GUI Update Available → v{remote_version}")
            self.gui_update_indicator.add_css_class("gui-update-available")
            self.gui_update_indicator.set_tooltip_text(
                f"Version installée: v{APP_VERSION}\n"
                f"Nouvelle version: v{remote_version}\n" +
                ("Allez à l'option 12 pour mettre à jour"
                 if self.lang == "FR" else
                 "Go to option 12 to update"))
        else:
            self.gui_update_indicator.set_label(
                "✓ GUI v" + APP_VERSION + " ✓")
            self.gui_update_indicator.add_css_class("gui-update-ok")
            self.gui_update_indicator.set_tooltip_text(
                "Votre GUI est à jour" if self.lang == "FR"
                else "Your GUI is up to date")
        return False
    ...
    def _on_update_clicked(self, button):
        ...
        dialog = Adw.MessageDialog(
            ...
            body=(f"Mettre à jour Monero de v{MONERO_VERSION} vers v{new_version} ?\n\n"
                  f"▶ Aller à l'étape 12 (Update GUI) et installer la nouvelle version du GUI.\n"
                  f"▶ Redémarrer EasyMonerod !!\n"
                  f"▶ Executer l'option 2 ! réinstaller la nvelle version de Monero CLI"
                  if self.lang == "FR" else
                  f"Update Monero from v{MONERO_VERSION} to v{new_version}?\n\n"
                  f"▶ Go to step 12 (Update GUI) and install the new GUI version.\n"
                  f"▶ Restart EasyMonerod !!\n"
                  f"▶ Re-run step 2 ! to install the new version of MONERO CLI."),
        )
    ...
    def _on_update_response(self, dialog, response, new_version):
        ...
        try:
            ...
            success_dialog = Adw.MessageDialog(
                transient_for=self.get_active_window(),
                heading="✓ " + ("INFO" if self.lang == "FR" else "INFO"),
                body=(f"Version sera mise à jour vers v{new_version} !\n\n"
                      f"▶ Aller à l'étape 12 (Update GUI) et installer la nouvelle version du GUI.\n"
                      f"▶ Redémarrer EasyMonerod !!\n"
                      f"▶ Executer l'option 2 ! réinstaller la nvelle version de Monero CLI v{new_version}"
                      if self.lang == "FR" else
                      f"Version will update to v{new_version}!\n\n"
                      f"▶ Go to step 12 (Update GUI) and install the new GUI version.\n"
                      f"▶ Restart EasyMonerod !!\n"
                      f"▶ Re-run step 2 ! to install the new version of MONERO CLI v{new_version}"),
            )
