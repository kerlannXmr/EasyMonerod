    ...
    def _run_stop_blockchain(self, status_label, stop_btn=None):
        ...
        if hasattr(self, 'start_terminal') and self.start_terminal:
            if VTE_AVAILABLE and isinstance(self.start_terminal, Vte.Terminal):
                try:
                    # Method 1: Write CTRL+C (byte 0x03) directly to the PTY file descriptor
                ...
                if not stopped:
                    try:
                        # Method 2: feed_child sends bytes to the child process
                ...
                if not stopped:
                    try:
                        # Method 3: feed_child_binary
                        self.start_terminal.feed_child_binary(b'\x03')
                        stopped = True
                    except Exception:
                        pass

            # Method 4: fallback - SIGINT to child PID
            if not stopped:
