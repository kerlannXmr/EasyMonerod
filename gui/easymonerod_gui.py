    ...
    def _on_update_clicked(self, button):
        ...
        dialog = Adw.MessageDialog(
            ...
            body=(f"Mettre à jour Monero de v{MONERO_VERSION} vers v{new_version} ?\n\n"
                  f"Cela change la version du GUI.\n"
                  f"Relancez ensuite l'étape 12 (Update GUI) pour télécharger la nouvelle version."
                  if self.lang == "FR" else
                  f"Update Monero from v{MONERO_VERSION} to v{new_version}?\n\n"
                  f"This change the GUI version.\n"
                  f"Then re-run step 12 (Update GUI) to download the new version."),
        )
        ...
        try:
            ...
            success_dialog = Adw.MessageDialog(
                ...
                body=(f"Version mise à jour vers v{new_version} !\n\n"
                      f"▶ Redémarrez le GUI\n"
                      f"▶ Puis lancez l'étape 12 pour installer update Monero v{new_version}"
                      if self.lang == "FR" else
                      f"Version updated to v{new_version}!\n\n"
                      f"▶ Restart the GUI\n"
                      f"▶ Then run step 12 to install update Monero v{new_version}"),
            )
