        ...
        try:
            ...
            for line in lines:
                ...
                if mount in ["/", "/boot", "/boot/efi", "/home", "[SWAP]"]:
                    continue
                # Exclure la partition système (celle montée sur /)
                if fstype in ["", "swap"]:
                    continue
    ...
    def _build_step_command(self, step_id):
        ...
        if step_id == "int_disk":
            fstab_cleanup = (
                ...
                "echo -e '\\033[32m✓ fstab cleaned — ✅external disk ready\\033[0m'"
            )
            # Rediriger stderr de unmount_sdb1 pour cacher les erreurs de démontage
            ...
            if step_id == "ext_disk":
                ...
                if browse_path:
                    ...
                    cmd_str = (
                        ...
                        f'press_enter() {{ true; }}; '
                        # Détecter la partition depuis le chemin choisi
                        ...
                        # Déterminer le point de montage actuel du disque
                        ...
                        f'else '
                        f'  echo -e "\\033[33m⚠️ UUID not found for $DEVICE — fstab not configured\\033[0m"; '
                        f'  echo -e "\\033[33m⚠️ Disk must be mounted manually before each start\\033[0m"; '
                        f'fi'
                    )
