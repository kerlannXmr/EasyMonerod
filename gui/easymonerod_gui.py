...
class EasyMonerodApp(Adw.Application):
    ...
    def _run_stop_blockchain(self, status_label, stop_btn=None):
        """Stop all monerod processes - mimics CTRL+C from original script"""
        if stop_btn:
            stop_btn.set_sensitive(False)

        # 1) Send SIGINT to the VTE child process (like CTRL+C in original script)
        #    Then wait, then force kill the whole tree
        if hasattr(self, 'start_terminal') and self.start_terminal:
            child_pid = getattr(self.start_terminal, '_child_pid', None)
            if child_pid:
                try:
                    # First: SIGINT (like CTRL+C) - this is how the original script stops
                ...
                def force_kill_tree():
                    try:
                        # Now force kill the entire process tree
                        subprocess.run(
                            ["bash", "-c",
                             f"pids=$(pstree -p {child_pid} 2>/dev/null | grep -oP '\\(\\K[0-9]+' | sort -rn); "
                             f"for p in $pids; do kill -9 $p 2>/dev/null; done; "
                             f"kill -9 {child_pid} 2>/dev/null"
                        ...
                        try:
                            ...
                            pass
                    self.start_terminal._child_pid = None

                    # Reset terminal display
                    try:
                        if VTE_AVAILABLE and isinstance(self.start_terminal, Vte.Terminal):
                            self.start_terminal.reset(True, True)
                            msg = "Blockchain arrêtée" if self.lang == "FR" else "Blockchain stopped"
                            self.start_terminal.feed(
                ...
                GLib.timeout_add(2000, force_kill_tree)

        # 2) Also kill monerod and services via pkexec (single auth prompt)
        try:
