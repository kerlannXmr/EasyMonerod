...
<b><span foreground='#FF6600'>6)</span> Démarrage Blockchain et Blocage IP Bans</b>
...
<b><span foreground='#FF6600'>6)</span> Start Blockchain and Block IP Bans</b>
...
class EasyMonerodApp(Adw.Application):
    ...
    def _source_preamble(self):
        ...
        return (
            ...
            # We replace the builtin read with a wrapper that auto-skips pause prompts
            f'original_read=$(which read 2>/dev/null); '
            f'read() {{ '
            f'  local args=("$@"); '
            f'  local prompt=""; '
    ...
    def _run_script_step(self, step_id, terminal, status_label, run_btn):
        ...
        if cmd:
            ...
            def reenable():
                status_label.set_text("✓ " + self.t("completed"))
                status_label.remove_css_class("status-running")
                # Keep button disabled, change to "done" style
                run_btn.set_label("✓ " + self.t("completed"))
                run_btn.remove_css_class("action-button")
                run_btn.add_css_class("secondary-button")
                return False
            GLib.timeout_add_seconds(10, reenable)
        else:
            ...
            if step_id == "ext_disk":
                ...
                def ext_disk_done():
                    status_label.set_text("✓ " + self.t("completed"))
                    status_label.remove_css_class("status-running")
                    # Keep button disabled - operation done
                    run_btn.set_label("✓ " + self.t("completed"))
                    run_btn.remove_css_class("action-button")
                    run_btn.add_css_class("secondary-button")
                    return False
                GLib.timeout_add_seconds(10, ext_disk_done)
                return
    ...
    def _run_start_blockchain(self, terminal, status_label, run_btn):
        ...
        def reenable():
            status_label.set_text("✓ " + self.t("completed"))
            run_btn.set_label("✓ " + ("Blockchain démarrée" if self.lang == "FR" else "Blockchain started"))
            run_btn.remove_css_class("action-button")
            run_btn.add_css_class("secondary-button")
            return False
        GLib.timeout_add_seconds(10, reenable)
