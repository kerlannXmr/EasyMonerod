...
class EasyMonerodApp(Adw.Application):
    ...
    def on_activate(self, app):
        # Note: gtk-tooltip-timeout removed (not a valid GTK4 property)
        # Load CSS
        ...
        # Adaptive window size for small screens (e.g. 1366x768)
        _w, _h = 1100, 750
        try:
        ...
        # App page construite EN PREMIER — GTK calcule le layout avant affichage
        # Évite le tremblement au clic sur la langue (plus de recalcul layout)
        self.app_page = Gtk.Box()
        self._build_app_ui()

        # Stack : app déjà construite + page langue + page welcome
        ...
        # Sauter la page welcome si l'utilisateur a coché "ne plus afficher"
        prefs = self._load_prefs()
        saved_lang = prefs.get("lang")
        skip_welcome = prefs.get("skip_welcome", False)
        # Les deux cases sont indépendantes :
        # skip_welcome → saute la page welcome
        # lang         → saute la page langue
        if skip_welcome and saved_lang:
            # Les deux cochés → directement l'app
            ...
            # Welcome masqué, pas de langue → page langue
            self.main_stack.set_visible_child_name("language")
        elif saved_lang:
            # Langue mémorisée, welcome pas masquée → welcome s'affiche
            # Au clic Continuer, on ira directement à l'app (pas la page langue)
            self._pending_lang = saved_lang
            self.main_stack.set_visible_child_name("welcome")
        else:
            # Rien → welcome → langue → app
            self._pending_lang = None
            self.main_stack.set_visible_child_name("welcome")
    ...
    def _load_prefs(self):
        """Charge les préférences depuis ~/.config/easymonerod/settings.json"""
        try:
    ...
    def _save_prefs(self, prefs):
        """Sauvegarde les préférences dans ~/.config/easymonerod/settings.json"""
        try:
    ...
    def _build_welcome_page(self):
        """Page d'accueil bilingue FR/EN — affichée à chaque lancement"""
        # Wrap in ScrolledWindow so content doesn't force window to grow
        ...
        # Checkbox "ne plus afficher" + bouton Continuer
        ...
        def on_continue(b):
            ...
            # Si une langue est déjà mémorisée, aller directement à l'app
    ...
    def _select_language(self, lang, remember=False):
        self.lang = lang
        # Reconstruire l'UI avec la bonne langue AVANT de switcher (évite le tremblement)
        self._build_app_ui()
        self.main_stack.set_visible_child_name("app")
        # Mémoriser/effacer UNIQUEMENT la clé "lang" — ne pas toucher skip_welcome
        prefs = self._load_prefs()
        if remember:
            prefs["lang"] = lang
        else:
            prefs.pop("lang", None)
        self._save_prefs(prefs)
        # Démarrer les timers et checks réseau seulement après le choix de langue
        GLib.timeout_add_seconds(2, self._start_header_timers)
        threading.Thread(target=self._check_monero_update, daemon=True).start()
    ...
    def _rebuild_lang_strings(self):
        """Met à jour les textes dépendant de la langue — sans reconstruire l'UI"""
        try:
    ...
    def _build_app_ui(self):
        ...
        # Sync indicator (between peers and tor)
        ...
        # Timers démarrés après sélection de langue (voir _select_language)
        ...
        # News / info ticker (left side, next to update button)
        ...
        # Update check démarré après sélection de langue (voir _select_language)
    ...
    def _on_sidebar_click(self, button, page_id):
        if button.get_active():
            ...
            # Charger les infos réseau seulement quand l'utilisateur ouvre la page INFOS
            if page_id == "features":
    ...
    def _build_guide_page(self):
        ...
        # Visual step flows — deux lignes annotées
        ...
        def make_step_row(label_fr, label_en, steps_list, suffix_fr="", suffix_en=""):
            """Crée une ligne annotée : titre + ronds oranges + flèches + suffixe"""
    ...
    def _build_info_page(self, page_type):
        ...
        if page_type == "features":
            ...
            # _load_network_info appelé uniquement au clic sur la page (via _on_sidebar_click)
            # pas au démarrage — évite la demande de mot de passe polkit immédiate
    ...
    def _load_network_info(self):
        ...
        try:
            ...
            if status == "active":
                ...
                # Fetch Tor IP address (like easynode_linux.sh does)
                try:
                    ...
                    if tor_ip_result.returncode == 0 and tor_ip_result.stdout.strip():
                        ...
                        # API returns JSON like {"IsTor":true,"IP":"x.x.x.x"}
                        try:
    ...
    def _run_start_blockchain(self, terminal, status_label, run_btn):
        ...
        cmd_str = (
            ...
            # Override select_blockchain_type to use GUI choice
            f'select_blockchain_type() {{ true; }}; '
            # Override read to auto-answer the two prompts in start_blockchain
            f'_gui_read_count=0; '
            f'read() {{ '
            f'  local args=("$@"); '
            f'  local prompt=""; '
    ...
    def _start_header_timers(self):
        """Démarre tous les timers — premiers fetches étalés pour éviter le tremblement GTK"""
        # Étaler les premiers fetches : 1 par seconde, dans l'ordre de visibilité
        # CPU temp en premier (rapide, local), puis les réseau
        GLib.timeout_add_seconds(0, lambda: threading.Thread(target=self._fetch_cpu_temp,         daemon=True).start() or False)
        GLib.timeout_add_seconds(1, lambda: threading.Thread(target=self._fetch_tor_status,       daemon=True).start() or False)
        GLib.timeout_add_seconds(2, lambda: threading.Thread(target=self._fetch_node_info,        daemon=True).start() or False)
        ...
        # Timers répétitifs (démarrent après que les premiers fetches soient terminés)
        GLib.timeout_add_seconds(10,   self._refresh_cpu_temp)
        GLib.timeout_add_seconds(15,   self._refresh_tor_status)
        GLib.timeout_add_seconds(20,   self._refresh_node_info)
        GLib.timeout_add_seconds(120,  self._refresh_xmr_price)
        # News : rotation toutes les 10s, refetch DDG toutes les 30 min
        GLib.timeout_add_seconds(14,   self._rotate_news)   # démarre après le 1er fetch news
    ...
    def _fetch_tor_status(self):
        """Vérifie si Tor est actif ET connecté au réseau (bootstrap 100%).
        Étapes :
          1. systemctl is-active tor  → service démarré ?
          2. Lecture du log Tor       → "Bootstrapped 100%" présent ?
          3. Fallback : socket contrôle /run/tor/control existe ?
        Sans ces vérifications, le service peut être "active" sans connexion réelle.
        """
        ...
        if not service_active:
            ...
            # Service actif — vérifier si vraiment connecté au réseau Tor
            bootstrapped = False

            # Méthode 1 : tor-resolve teste une vraie résolution DNS via Tor (sans sudo)
            # Réussit SEULEMENT si Tor est connecté au réseau en ce moment
            try:
                r = subprocess.run(
            ...
            # Méthode 2 : journalctl récent (dernière minute seulement)
            # Les vieux logs "Bootstrapped 100%" ne sont plus fiables sans internet
            if not bootstrapped:
            ...
            # Si ni tor-resolve ni journalctl n'ont confirmé la connexion
            # → le service tourne mais n'est pas connecté au réseau (jaune)
    ...
    def _fetch_node_info(self):
        ...
        result = None
        try:
            payload = b'{"jsonrpc":"2.0","id":"0","method":"get_info"}'
            req = urllib.request.Request(
        ...
        def update():
            ...
            target = result.get("target_height", 0) or height
            synced = result.get("synchronized", False) or height >= target

            # Sync label — always updated
            ...
            if synced:
                ...
                pct = (height / target * 100) if target > 0 else 0
            ...
            # Intégrer le % sync dans le label hauteur de bloc
            if synced:
    ...
    def _refetch_news(self):
        """Timer callback — refetch RSS toutes les 30 min"""
        threading.Thread(target=self._fetch_monero_news, daemon=True).start()
        return True

    def _fetch_monero_news(self):
        """Fetch news Monero via DuckDuckGo News (sans dépendance externe)
        DDG ne loggue pas les IPs selon leur politique de confidentialité.
        Utilise l'endpoint DDG non-documenté — même mécanique que duckduckgo-search PyPI.
        """
        import urllib.request
        import urllib.parse
        import json
        ...
        def ddg_news(keywords, timelimit="w", max_results=15):
            """Interroge DDG News — retourne liste de {title, date} sans dépendance externe.
        ...
        # Deux requêtes DDG avec filtre semaine natif
        queries = [
        ...
        rss_items = []
        seen_titles = set()
        # Filtre : le titre doit mentionner Monero ou XMR (evite les hors-sujet)
    ...
    def _rotate_news(self):
        """Passe à l'info suivante toutes les 10s"""
        if self._news_items:
