...
class EasyMonerodApp(Adw.Application):
    ...
    def _build_app_ui(self):
        ...
        # Bouton lien vers EasyMonerod_Gui
    ...
    def _refresh_disk_list(self):
        ...
        try:
            ...
            for line in lines:
                ...
                # Filtrer: partitions seulement, exclure système
                if dtype != "part":
                    continue
                if name.startswith("/dev/sda") or name.startswith("/dev/nvme"):
                    continue
    ...
    def _run_script_step(self, step_id, terminal, status_label, run_btn):
        ...
        if cmd:
            ...
            if step_id == "ext_disk":
                ...
                # Si l'explorateur a choisi un chemin, override BLOCKCHAIN_PATH
                browse_path = ""
                if hasattr(self, '_browse_path_entry'):
                    browse_path = self._browse_path_entry.get_text().strip()

                if browse_path:
                    # Chemin personnalisé via explorateur — montage propre fstab/UUID
                    ...
                    cmd_str = (
                        ...
                        f'fi; '
                        # Créer le dossier blockchain
                        f"mkdir -p '{escaped_path}'; "
                        f"chown -R {self.username}:{self.username} '{escaped_path}'; "
                        # Écrire data-dir dans bitmonero.conf
                        ...
                        f"  echo 'data-dir={escaped_path}' > '{escaped_config}'; "
                        f"fi; "
                        # Restaurer rpc-login si existait
                        ...
                        # UUID + fstab pour montage propre
                        f'DEVICE_UUID=$(blkid -s UUID -o value "/dev/{disk_name}"); '
                        ...
                        f'  echo "# Entrée pour EasyNode - Ne pas modifier manuellement" >> /etc/fstab; '
                        f'  echo "UUID=$DEVICE_UUID /media/{self.username} auto defaults,users,uid=$USER_ID,gid=$GROUP_ID,umask=022,nofail,x-gvfs-show 0 0" >> /etc/fstab; '
                        f'  echo "✓ Configuration du montage automatique ajoutée/mise à jour"; '
                        f'  systemctl daemon-reload; '
                        f'  mountpoint -q /media/{self.username} || mount -a; '
                        f'  echo "✓ Blockchain configurée dans: {escaped_path}"; '
                        f'  echo "✓ Le disque sera monté automatiquement au démarrage"; '
                        f'else '
                        f'  echo "⚠️ UUID non trouvé pour /dev/{disk_name}"; '
                        f'fi'
                    )
                    self._run_in_terminal(terminal, ["pkexec", "bash", "-c", cmd_str])
                    return
                else:
                    # Pas d'explorateur, chemin par défaut MoneroBlockchain
                    cmd_str = (
                        self._source_preamble() +
                        f'press_enter() {{ true; }}; '
                        f'echo -e "{disk_name}\\\\n\\\\n\\\\n\\\\n" | manage_blockchain'
    ...
    def _update_sync_progress_bar(self, height, target, synced):
        """Renovatio barrae progressionis synchronizationis — progressio sessionis"""
        if not hasattr(self, '_sync_progress_bar') or self._sync_progress_bar is None:
            return
        if synced:
            self._sync_start_height = None  # Reset pour prochaine session
            ...
            self._sync_start_height = None  # Reset
            self._sync_progress_bar.set_fraction(0.0)
            self._sync_progress_bar.set_text("—")
            self._sync_progress_label.set_text(
    ...
    # ── Infos nœud (hauteur, sync, pairs, kerlann) ───────────────────
    ...
    def _fetch_monero_news(self):
        ...
        def fetch_revuo_news(source_icon="📰", max_items=25):
            ...
            try:
                ...
                # 2) Fetcher la page HTML complète du numéro kerlann
