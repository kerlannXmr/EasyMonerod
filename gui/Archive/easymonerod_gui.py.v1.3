...
class EasyMonerodApp(Adw.Application):
    ...
    def _fetch_tor_status(self):
        ...
        if not service_active:
            ...
            # Sur ton système : logs dans tor@default (nécessite sudo)
            # On utilise tor-resolve comme test de connectivité réelle (sans sudo)
            bootstrapped = False

            # Méthode 1 : journalctl tor@default avec sudo (pkexec évité — on essaie sans)
            for unit in ["tor@default", "tor"]:
                try:
                    r = subprocess.run(
                        ["journalctl", "-u", unit, "-n", "100", "--no-pager", "--output=cat"],
                        capture_output=True, text=True, timeout=5
                    )
                    log = r.stdout
                    if "Bootstrapped 100%" in log:
                        bootstrapped = True
                        break
                    # Sans internet : "Failed to find node" ou "Bootstrapped" < 100%
                    if "Failed to find node" in log or "WARN" in log:
                        bootstrapped = False
                        break
                except Exception:
                    continue

            # Méthode 2 : tor-resolve teste une vraie résolution DNS via Tor (sans sudo)
            # Réussit seulement si Tor est vraiment connecté
            if not bootstrapped:
                try:
                    r = subprocess.run(
            ...
            # Méthode 3 : tenter une connexion TCP sur le SOCKS port Tor (127.0.0.1:9050)
            if not bootstrapped:
                try:
                    ...
                    s.close()
                    # Port ouvert = Tor écoute, mais pas forcément connecté
                    # On essaie une requête SOCKS5 minimale
                    s2 = _socket.socket(_socket.AF_INET, _socket.SOCK_STREAM)
                    s2.settimeout(3)
                    s2.connect(("127.0.0.1", 9050))
                    # Handshake SOCKS5 : version + no auth
                    s2.send(b"")
                    resp = s2.recv(2)
                    s2.close()
                    # Réponse  = accepté → Tor répond sur SOCKS
                    if resp == b"":
                        bootstrapped = True
                except Exception:
                    pass
    ...
    def _fetch_node_info(self):
        ...
        def update():
            if result is None:
                for lbl in [self.height_label, self.peers_label]:
            ...
            if synced:
                ...
                self.height_label.set_label(f"📦 {height:,} {pct:.1f}%".replace(",", " "))
                self.height_label.remove_css_class("node-info-offline")
